...
tokens = tokens = [w.strip(".,!?;:") for w in text.split()] # Your code here
...
def tokenize(string: str) -> list:
    clean = "".join(
        ch.lower() if (ch.isalnum() or ch in "\n\t ") else " "
        for ch in string
    )
    words = clean.split()
    return words
...
# Your code here:
# -----------------------------------------------
word_frequencies = {w.lower(): [t.lower() for t in tokens].count(w.lower()) for w in tokens}
...
def token_counts(string: str, k: int = 1) -> dict:
    ...
    freq = {w: tokens.count(w) for w in set(tokens)}
    return {w: c for w, c in freq.items() if c > k}
...
# Your code here:   
# -----------------------------------------------
token_to_id = {tok: idx for idx, tok in enumerate(sorted(set(tokens)))}
...
def tokenize_and_encode(documents: list) -> list:
    ...
    for doc in documents:
        toks = tokenize(doc)
        encoded = [token_to_id[t] for t in toks]
